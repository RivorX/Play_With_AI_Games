...
def analyze_conv_channels_detailed(model, env, output_dir, num_samples=100):
    """
    Analizuje aktywację kanałów Conv1-Conv4 na różnych obserwacjach
    ✅ UPDATED: 4-layer CNN (bez LSTM)
    ...
    """
    print("\n" + "="*70)
    print("🔬 ANALYZING ALL CNN CHANNELS (Conv1/Conv2/Conv3/Conv4)")
    print("="*70 + "\n")
    ...
    # Detect number of channels for each layer (4 layers)
    num_ch1 = features_extractor.conv_layers[0].out_channels
    num_ch2 = features_extractor.conv_layers[1].out_channels
    num_ch3 = features_extractor.conv_layers[2].out_channels
    num_ch4 = features_extractor.conv_layers[3].out_channels
    
    print(f"📐 Architecture: Conv1({num_ch1}) → Conv2({num_ch2}) → Conv3({num_ch3}) → Conv4({num_ch4})\n")
    
    # Storage for channel statistics (per layer)
    layers_stats = {}
    for layer_name, num_channels in [
        ('conv1', num_ch1), ('conv2', num_ch2), 
        ('conv3', num_ch3), ('conv4', num_ch4)
    ]:
    ...
    for sample_idx in range(num_samples):
        ...
        with torch.no_grad():
            ...
            # Conv1
            x = features_extractor.conv_layers[0](x)
            x = features_extractor.bn_layers[0](x)
            conv1_out = F.gelu(x)
            conv1_out_cpu = conv1_out.cpu().numpy()[0]
            
            # Conv2
            x = features_extractor.conv_layers[1](conv1_out)
            x = features_extractor.bn_layers[1](x)
            conv2_out = F.gelu(x)
            conv2_out_cpu = conv2_out.cpu().numpy()[0]
            
            # Conv3 (stride=2)
            x = features_extractor.conv_layers[2](conv2_out)
            x = features_extractor.bn_layers[2](x)
            conv3_out = F.gelu(x)
            conv3_out_cpu = conv3_out.cpu().numpy()[0]
            
            # Conv4
            x = features_extractor.conv_layers[3](conv3_out)
            x = features_extractor.bn_layers[3](x)
            conv4_out = F.gelu(x)
            conv4_out_cpu = conv4_out.cpu().numpy()[0]
            ...
            for ch in range(num_ch3):
                ch_data = conv3_out_cpu[ch]
                layers_stats['conv3']['mean'][ch] += ch_data.mean()
                layers_stats['conv3']['max'][ch] += ch_data.max()
                layers_stats['conv3']['std'][ch] += ch_data.std()
                layers_stats['conv3']['active_ratio'][ch] += (ch_data > 0.1).mean()
            
            for ch in range(num_ch4):
                ch_data = conv4_out_cpu[ch]
                layers_stats['conv4']['mean'][ch] += ch_data.mean()
                layers_stats['conv4']['max'][ch] += ch_data.max()
                layers_stats['conv4']['std'][ch] += ch_data.std()
                layers_stats['conv4']['active_ratio'][ch] += (ch_data > 0.1).mean()
    ...
    for idx, layer_name in enumerate(['conv1', 'conv2', 'conv3', 'conv4']):
        stats = layers_stats[layer_name]
        num_channels = len(stats['mean'])
        ...
        ax1 = plt.subplot(4, 4, base_pos + 1)
        ...
        # Plot 2: Active ratio
        ax2 = plt.subplot(4, 4, base_pos + 2)
        ...
        ax3 = plt.subplot(4, 4, base_pos + 3)
        ...
        ax4 = plt.subplot(4, 4, base_pos + 4)
