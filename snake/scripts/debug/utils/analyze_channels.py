...
Analizuje aktywację każdego kanału Conv1/Conv2/Conv3 na różnych obserwacjach
...
def analyze_conv_channels_detailed(model, env, output_dir, num_samples=100):
    """
    Analizuje aktywację kanałów Conv1, Conv2, Conv3 na różnych obserwacjach
    ✅ UPDATED: 3-layer hybrid CNN (5×5 → 3×3 → 3×3) with GroupNorm
    ...
    """
    print("\n" + "="*70)
    print("🔬 ANALYZING ALL CNN CHANNELS (Conv1/Conv2/Conv3)")
    print("="*70 + "\n")
    ...
    num_ch3 = features_extractor.conv3.out_channels
    
    print(f"📐 Architecture: Conv1({num_ch1}) → Conv2({num_ch2}) → Conv3({num_ch3})\n")
    
    # Storage for channel statistics (per layer)
    layers_stats = {}
    for layer_name, num_channels in [('conv1', num_ch1), ('conv2', num_ch2), ('conv3', num_ch3)]:
    ...
    for sample_idx in range(num_samples):
        ...
        with torch.no_grad():
            ...
            conv2_out = F.silu(x)
            conv2_out_cpu = conv2_out.cpu().numpy()[0]  # Shape: (num_ch2, H, W)
            
            # Conv3 (3×3, strided)
            x = features_extractor.conv3(conv2_out)
            x = features_extractor.norm3(x)
            x = F.silu(x)
            conv3_out = features_extractor.dropout3(x)
            conv3_out_cpu = conv3_out.cpu().numpy()[0]  # Shape: (num_ch3, H/2, W/2)
    ...
    for idx, layer_name in enumerate(['conv1', 'conv2', 'conv3']):
        ...
        ax1 = plt.subplot(3, 4, base_pos + 1)
        ...
        # Plot 2: Active ratio
        ax2 = plt.subplot(3, 4, base_pos + 2)
        ...
        ax3 = plt.subplot(3, 4, base_pos + 3)
        ...
        ax4 = plt.subplot(3, 4, base_pos + 4)
