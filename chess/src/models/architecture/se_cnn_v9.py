...
class SECNNV9(nn.Module):
    ...
    def __init__(self, config, input_planes=None):
        ...
        self.architecture_id = "se_cnn_v9"
        self.model_config = dict(config.get("model", {}))
        from src.models.contracts import model_spec_from_config
        self.model_spec = model_spec_from_config(config)
        ...
        # head as an auxiliary representation-learning objective.
    ...
    def _print_parameter_summary(self, num_blocks):
        ...
        search_value_params = self._count_parameters(self.search_q_fc)
        moves_left_params = (
        ...
        print(f"    - Search-Q auxiliary head: {search_value_params:,}")
    ...
    def forward(
        ...
        if return_search_aux:
            return policy, value, torch.tanh(search_q_raw)
