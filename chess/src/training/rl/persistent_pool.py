...
from src.inference.central import central_inference_server
from src.selfplay.workers import persistent_selfplay_worker
from src.models.data.se_cnn_v9.helpers import MAX_LEGAL_MOVES
from src.inference.shared_memory import (
...
def _close_mp_queue(queue_obj):
    """Close a multiprocessing queue without waiting for a blocked feeder.

    During interrupted dynamic self-play the parent can still have hundreds of
    game descriptors buffered locally after every worker has been terminated.
    ``multiprocessing`` normally joins that feeder thread at interpreter exit,
    which can hang forever because the queue pipe no longer has a reader.
    Dropping the unsent tail is the correct shutdown behavior.
    """
...
def _resolve_central_inference_server_count(config, worker_specs, device_type):
    ...
    # Keep at most one central server per visible GPU. Competing server
    # processes on one device split batches and add context-transfer overhead.
...
class _PersistentSelfPlayPool:
    def __init__(self, config, worker_specs, device_type, temp_dir):
        ...
        # Bound packed replay chunks. Queue.put then applies backpressure instead
        # of letting every worker retain arbitrarily many serialized game batches.
        self.result_queue = self.mp_ctx.Queue(maxsize=max(4, 2 * len(self.worker_specs)))
        # One inherited queue of individual game descriptors lets every worker
        # refill a freed game slot immediately.  This removes fixed per-process
        # quotas and leaves only one global drain wave at the end of an iteration.
        self.game_queue = self.mp_ctx.Queue()
        self.task_queues = {}
        self.processes = {}
        rl_cfg = config.get('reinforcement_learning', {})
        central_cfg = config.get('central_inference', {}) or {}
    ...
    def dispatch_task(
        ...
        self.task_queues[rank].put({
            ...
            'stream_game_queue': bool(stream_game_queue),
            'initial_game_jobs': list(initial_game_jobs or []),
        })
        return result_file, progress_file

    def prepare_game_stream(self, task_id, jobs):
        """Publish all jobs before workers start, followed by one stop token each."""
        while True:
            try:
                self.game_queue.get_nowait()
            except queue.Empty:
                break
        for job in list(jobs or []):
            payload = dict(job or {})
            payload['task_id'] = str(task_id)
            self.game_queue.put(payload)
        for _rank, _games in self.worker_specs:
            self.game_queue.put({'cmd': 'end_game_stream', 'task_id': str(task_id)})
    ...
    def shutdown(self, timeout_s=5):
        ...
        # Disarm queue feeder joins before killing their readers. In particular,
        # game_queue can still contain most of an interrupted 1000-game stream.
        queues = [
