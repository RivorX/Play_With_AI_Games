...
class ILLoggerMixin:
    ...
    def _load_il_plot_history(self, rows):
        for row in rows:
            ...
            for simulations, sim_entry in sorted(
                _parse_mcts_elo_by_simulations(
                ...
                self.record_il_mode_elo(
                    ...
                    label=f"Final MCTS @{simulations}",
                    update_csv=False,
                    std_error=sim_entry.get("se"),
                    ci95=sim_entry.get("ci95"),
                )
        ...
        *,
        ...
        for idx, existing in enumerate(self.il_mode_elo_markers):
            if (
                ...
                and (
                    mode == "nn"
                    or int(existing.get("simulations", 0) or 0) == simulations
                )
            ):
        ...
        try:
            ...
            if mode == "mcts":
                ...
                target_row[by_sims_idx] = _upsert_mcts_elo_by_simulations(
                    target_row[by_sims_idx],
                    simulations,
                    elo_value,
                    std_error=std_error,
                    ci95=(ci_low, ci_high),
    ...
    def _plot_il_elo_panel(self, ax):
        ...
        if has_mode_markers:
            ...
            mcts_simulations = sorted({
                int(marker.get("simulations", 0) or 0)
                for marker in self.il_mode_elo_markers
                if marker.get("mode") == "mcts"
                and int(marker.get("simulations", 0) or 0) > 0
            })
            mcts_colors = {
                simulations: mcts_palette[index % len(mcts_palette)]
            ...
            for marker_info in sorted(markers_to_plot, key=lambda item: (int(item.get("epoch", 0)), str(item.get("mode", "")), str(item.get("label", "")))):
                try:
                    ...
                    continue
                marker_mode = marker_info.get("mode", "nn")
                style = dict(mode_styles.get(marker_mode, mode_styles["nn"]))
                sims = marker_info.get("simulations", 0) or 0
                try:
                    sims = int(sims)
                except (TypeError, ValueError):
                    sims = 0
                if marker_mode == "mcts" and sims in mcts_colors:
                    style["color"], style["edge"] = mcts_colors[sims]
    ...
    def _log_il(self, iteration, train_losses=None, val_losses=None,
        ...
        row.extend([
            ...
            kwargs.get('estimated_elo_mcts_by_simulations', ''),
