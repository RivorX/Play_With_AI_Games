...
from src.model import create_model, normalize_state_dict_keys
from src.evaluation.elo_runner import run_elo_check
...
def _async_elo_worker(
    ...
    try:
        ...
        worker_model = create_model(worker_config).to(worker_device)
...
class ILEloCoordinator:
    ...
    def __init__(self, model, config, device, elo_config_il, logger):
        ...
        self.enabled = self.eval_every > 0
        if not self.enabled:
            self.eval_every = 0
        ...
        self.last_estimated_elo = None
        try:
            _, previous_elo = self.logger.get_latest_estimated_elo_with_epoch()
            if previous_elo is not None:
                self.last_estimated_elo = float(previous_elo)
        except Exception:
            pass
    ...
    def print_startup_summary(self, verbose=False):
        ...
        print(
            ...
            f"initial focus={self.elo_config.get('adaptive_focus_games_per_level', 32)}/level, "
        ...
        while True:
            ...
            if payload.get("cancelled"):
                ...
                if estimated_elo is not None:
                    self.last_estimated_elo = float(estimated_elo)
    ...
    def start_async(self, epoch_num):
        ...
        if self.last_estimated_elo is not None:
            worker_elo_cfg["adaptive_initial_elo"] = float(self.last_estimated_elo)
            worker_elo_cfg["adaptive_initial_elo_source"] = "previous IL Elo measurement"
        payload = {
            "epoch": int(epoch_num),
    ...
    def evaluate_if_due(self, epoch_num):
        ...
        runtime_elo_config = dict(self.elo_config)
        if self.last_estimated_elo is not None:
            runtime_elo_config["adaptive_initial_elo"] = float(self.last_estimated_elo)
            runtime_elo_config["adaptive_initial_elo_source"] = "previous IL Elo measurement"
        elo_result = run_elo_check(self.model, self.config, self.device, runtime_elo_config)
