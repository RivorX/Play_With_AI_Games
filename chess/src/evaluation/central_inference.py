...
class CentralInferenceSession:
    ...
    def start(self):
        ...
        transport_dtype = str(central_cfg.get("transport_dtype", "float16")).lower()
        shared_memory_enabled = bool(
            self._option("central_inference_shared_memory_enabled", True)
            and transport_dtype in {"float16", "fp16"}
        )
        max_batch = max(1, int(central_cfg.get("max_batch_size", 256) or 256))
        slot_count = max(
            1,
            int(self._option("central_inference_shared_memory_slots_per_worker", 1) or 1),
        )
        input_planes = 16 * (
            1 + int(self.config.get("model", {}).get("history_positions", 0) or 0)
        )
        shared_buffers_by_rank = {
        ...
        for server_idx in range(server_count):
            ...
            process = mp_ctx.Process(
                ...
                args=(
                    ...
                    response_queues,
                    control_queue,
                    self.rank_base + server_idx,
                    {
                        int(rank): shared_buffers_by_rank[int(rank)]
                        for rank in response_queues
                        if int(rank) in shared_buffers_by_rank
                    },
                ),
            )
            process.daemon = True
        ...
        self._session = {
            ...
            "shared_buffers_by_rank": shared_buffers_by_rank,
        }
        return self
    ...
    def remote_model_for_current_thread(self, model_label: str | None = None):
        ...
        return RemoteInferenceClient(
            ...
            shared_buffer=(self._session.get("shared_buffers_by_rank", {}) or {}).get(rank),
        )
    ...
    def describe(self) -> str:
        ...
        shared_bytes = sum(
            shared_inference_bytes(spec)
            for spec in (session.get("shared_buffers_by_rank", {}) or {}).values()
        )
        if shared_bytes > 0:
            load_summary += f", shared_memory={shared_bytes / (1024 ** 2):.1f} MiB"
        return (
