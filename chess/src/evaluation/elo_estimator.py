...
from src.evaluation.central_inference import CentralInferenceSession, snapshot_model_state_cpu
...
def ensure_stockfish(configured_path: str = "stockfish") -> str:
    ...
    chess_dir = Path(__file__).resolve().parents[2]
...
class _ModelPlayer:
    ...
    def __init__(self, model, config, device, use_mcts: bool = False, simulations: int = 100):
        ...
        self.mcts = None
        if use_mcts:
            self.mcts = SingleGameMCTS(model, config, device)
...
class _BatchedModelPlayer:
    ...
    def create_state(self) -> dict:
        return {
            ...
            "root": None,
            "_root_synced": False,
            "_native_tree_key": object(),
        }

    def reset_state(self, state: dict):
        if self.multi_mcts is not None:
            self.multi_mcts.release_native_tree(state.get("_native_tree_key"))
        state["board_history"] = []
        state["native_board"] = None
        state["root"] = None
        state["_root_synced"] = False
        state["_native_tree_key"] = object()
            ...
            game_states.append(
                {
                    ...
                    "_native_tree_key": state.setdefault("_native_tree_key", object()),
                }
            )

        search_results = self.multi_mcts.search_results_many(
            game_states,
...
class EloEstimator:
    ...
    def estimate(
        ...
        configured_initial_elo = self.elo_config.get("adaptive_initial_elo")
        if levels is None:
            levels = build_stockfish_elo_levels(
                sf_min_elo,
                sf_max_elo,
                initial_elo=configured_initial_elo,
            )
            print(
                f"  Info: Stockfish UCI_Elo range: {sf_min_elo}-{sf_max_elo}; "
                f"auto ladder has {len(levels)} candidate levels."
            )
        else:
            # Explicit levels remain supported for isolated benchmarks/tests,
            # but normal IL/RL/manual evaluation always uses engine discovery.
            valid_levels = []
            for lvl in levels:
                clamped = max(sf_min_elo, min(sf_max_elo, int(lvl)))
                if clamped != lvl:
                    print(
                        f"  Warning: Clamped level {lvl} -> {clamped} "
                        f"(SF range: {sf_min_elo}-{sf_max_elo})"
                    )
        ...
        try:
            hard_max_total_games = int(
                self.elo_config.get("adaptive_hard_max_total_games", max_total_games)
                or max_total_games
            )
        except (TypeError, ValueError):
            hard_max_total_games = max_total_games
        hard_max_total_games = max(max_total_games, hard_max_total_games)
        try:
            budget_extension_games = int(
                self.elo_config.get("adaptive_budget_extension_games", extra_round_cfg * 6)
                or extra_round_cfg * 6
            )
        except (TypeError, ValueError):
            budget_extension_games = extra_round_cfg * 6
        ...
        print(
            ...
            f"games=adaptive {max_total_games} initial / {hard_max_total_games} hard cap"
        )
        print(
        ...
        try:
            if schedule.initial_elo is not None:
                seed_source = str(
                    self.elo_config.get("adaptive_initial_elo_source", "previous checkpoint Elo")
                    or "previous checkpoint Elo"
                )
                print(
                    f"  Previous Elo: {schedule.initial_elo:.0f} ({seed_source}); "
                    "used only to choose opponents, not as a rating result."
                )
            if central_inference_enabled:
            ...
            for wave_levels in schedule.probe_waves():
                if self._is_cancelled() or schedule.total_scheduled >= schedule.max_total_games:
                    break
            ...
            while (
                ...
                and schedule.total_scheduled < schedule.max_total_games
            ...
            while (
                ...
                if schedule.total_scheduled >= schedule.max_total_games:
                    if schedule.max_total_games >= hard_max_total_games:
                        break
                    old_limit = schedule.max_total_games
                    new_limit = min(
                        hard_max_total_games,
                        old_limit + budget_extension_games,
                    )
                    schedule.extend_game_budget(new_limit)
                    current_se = schedule.current_standard_error()
                    se_text = f"SE={float(current_se):.1f}" if current_se is not None else "SE not ready"
                    print(
                        "  Adaptive Elo: evidence is still insufficient "
                        f"({se_text}); extending game budget {old_limit} -> {new_limit}."
                    )
            ...
            if (
                ...
                and schedule.target_se > 0.0
                and not schedule.precise_enough()
                and schedule.total_scheduled >= hard_max_total_games
            ):
                current_se = schedule.current_standard_error()
                if current_se is not None and float(current_se) > schedule.target_se:
                    print(
                        "  Adaptive Elo: reached hard game cap "
                        f"({hard_max_total_games}) with SE={float(current_se):.1f} "
        ...
        result = {
            ...
            "initial_elo": round(schedule.initial_elo) if schedule.initial_elo is not None else None,
            "initial_game_budget": int(max_total_games),
...
def estimate_model_elo(
    ...
    Args:
        model: chess policy/value model in eval mode.
