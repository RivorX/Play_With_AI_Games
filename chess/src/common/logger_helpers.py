...
def _parse_mcts_elo_by_simulations(raw_value):
    ...
    normalized = {}
    for raw_simulations, raw_entry in payload.items():
        try:
            simulations = max(1, int(raw_simulations))
        except (TypeError, ValueError):
            continue
        entry = dict(raw_entry) if isinstance(raw_entry, dict) else {"elo": raw_entry}
        elo = _clean_optional_float(entry.get("elo"))
        if elo is None:
            continue
        clean_entry = {"elo": float(elo)}
        standard_error = _clean_optional_float(entry.get("se"))
        if standard_error is not None:
            clean_entry["se"] = float(standard_error)
        ci_low, ci_high = _clean_elo_ci95(entry.get("ci95"))
        if ci_low is not None and ci_high is not None:
            clean_entry["ci95"] = [float(ci_low), float(ci_high)]
        normalized[int(simulations)] = clean_entry
    return normalized


def _upsert_mcts_elo_by_simulations(raw_value, simulations, elo, *, std_error=None, ci95=None):
    try:
        simulations = max(1, int(simulations))
        elo = float(elo)
    except (TypeError, ValueError):
        return str(raw_value or "")
    payload = _parse_mcts_elo_by_simulations(raw_value)
    entry = {"elo": elo}
    standard_error = _clean_optional_float(std_error)
    if standard_error is not None:
        entry["se"] = float(standard_error)
    ci_low, ci_high = _clean_elo_ci95(ci95)
    if ci_low is not None and ci_high is not None:
        entry["ci95"] = [float(ci_low), float(ci_high)]
    payload[simulations] = entry
    return json.dumps(
        {str(key): payload[key] for key in sorted(payload)},
        separators=(",", ":"),
    )
