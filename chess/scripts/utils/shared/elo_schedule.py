...
from utils.shared.elo_rating import elo_fit_diagnostics, expected_score, performance_rating
...
class AdaptiveEloSchedule:
    ...
    def _interleaved_order(levels: list[int]) -> list[int]:
        ...
        for offset in range(n):
            for index in (offset, n - 1 - offset, middle + offset, middle - offset):
                if 0 <= index < n and index not in seen:
                    seen.add(index)
                    indices.append(index)
            if len(indices) >= n:
                break
        return [levels[index] for index in indices]

    @property
    def probe_order(self) -> list[int]:
        if self.initial_elo is None:
            return self._interleaved_order(self.levels)
        return sorted(self.levels, key=lambda level: (abs(float(level) - self.initial_elo), level))
