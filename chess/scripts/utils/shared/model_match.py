...
def run_model_match(
    ...
    *,
    ...
    """
    # Keep Pygame/setup imports out of Windows spawn workers. This helper is
    # needed only once in the parent-side match coordinator.
    from utils.ui.game_setup import _infer_architecture_from_state_dict

    match_config = copy.deepcopy(config)
    match_config.setdefault("model", {})
    match_config["model"].update(_infer_architecture_from_state_dict(model1.state_dict()))
    ...
    same_architecture = bool(_state_shapes(model1) == _state_shapes(model2))
    ...
    if central:
        ...
        max_batch = max(1, int(central_cfg.get("max_batch_size", batch_games) or batch_games))
        # The central model is compiled with dynamic shapes. The broad RL
        # warm-up (1, 32, 128, 256, 512) is useful for a long training run, but
        # wastes startup time in a 100-game GUI match. Warm only latency and the
        # actual per-worker match batch; larger aggregate batches remain valid.
        central_cfg["compile_warmup_batches"] = sorted({1, min(batch_games, max_batch)})
