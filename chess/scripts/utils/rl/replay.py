...
# Guarded actor self-play keeps the old numeric codes for checkpoint/log
# compatibility.  The aliases describe their current meaning.
REPLAY_SOURCE_ACTOR = REPLAY_SOURCE_LEARNER
...
class ReplayBuffer:
    ...
    def select_iteration_indices(
        ...
        *,
        ...
        """Return positions inserted by one actor generation.
    ...
    def select_hard_position_indices(self, sample_size, min_age=0):
        """Importance-weighted positions that can be reconstructed as native boards."""
    ...
    def update_reanalyzed_targets(
        ...
        *,
        ...
        for row, idx in enumerate(np.asarray(indices, dtype=np.int64).tolist()):
            ...
            # The mixed target no longer has a one-to-one relationship with the
            # search decision originally stored for this row.  Clear the old
            # correction label so correction uptake telemetry stays truthful.
